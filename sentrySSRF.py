#!/usr/bin/env python
# Python 3
# Sentry SSRF By Danila_xawdxawdx using LinkFinder
...
def cli_output(endpoints):
    ...
    for endpoint in endpoints:
        s = html.escape(endpoint["link"]).encode(
            'ascii', 'ignore').decode('utf8')
        # print(s)
        if len(s) > 40:
            if s[40] == '@':
                print("Found Sentry: " + s)
                collaborator = input("Enter your burpcollaborator address:")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--domain",
                        help="Input a domain to recursively parse all javascript located in a page to find config with sentry",
                        action="store_true")
    parser.add_argument("-i", "--input",
                        help="Input an exact url to parse",
